...
scan_expiry = {}  # Track scan data expiry
multi_scan_selections = {}  # Store multi-scan selections
...
# Nmap scan function (single or multi)
async def nmap_scan(ip, chat_id, update, context, scan_types):
    ...
    scan_expiry[chat_id] = time.time() + 600  # 10 min expiry

    nm = nmap.PortScanner()
    # Combine scan types, avoid conflicts (e.g., sT and sS)
    scan_args = []
    scan_types = list(set(scan_types))  # Remove duplicates
    if "sT" in scan_types and "sS" in scan_types:
        scan_types.remove("sS")  # sT takes precedence over sS
    if "A" in scan_types:
        scan_args.append("-A")  # Aggressive includes sV, sC, O
    else:
        for st in scan_types:
            scan_args.append(f"-{st}")
    args = " ".join(scan_args) + " -Pn -T4"  # -Pn to skip ping, T4 for speed
    ...
    try:
        if query.data.startswith("ip_scan_"):
            ...
            if action == "open":
                ...
                # Only clear scan_results if user explicitly starts new scan
