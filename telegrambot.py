...
scan_expiry = {}
multi_scan_selections = {}
last_command_time = {}  # Kept for potential future use
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔧 Multi-Nmap Scan", callback_data=f"multi_nmap_{chat_id}")]
    ]
    ...
    await update.message.reply_text(
        ...
        "🔍 **Nmap Advanced Scan**: Single Nmap scan (TCP, Version, etc.)\n"
        "🔧 **Multi-Nmap Scan**: Combine up to 2 Nmap scans (e.g., TCP + Version)",
        parse_mode="Markdown",
...
# Nmap scan function
async def nmap_scan(ip, chat_id, update, context, scan_types):
    ...
    async with scan_semaphore:
        try:
            ...
            lock_timeouts[chat_id] = time.time() + 900  # 15 min timeout
            ...
            # Validate scan types
            if "sT" in scan_types and "sS" in scan_types:
                scan_types.remove("sS")
                logger.info("Removed -sS as -sT is selected")
            if len(scan_types) > 2:
                scan_types = scan_types[:2]
                logger.info(f"Limited to 2 scan types: {scan_types}")
            for st in scan_types:
                scan_args.append(f"-{st}")
            args = " ".join(scan_args) + " -Pn -T4 --top-ports 1000 --min-rate 50 --max-rate 200"
            eta_text = "1-5 min"

            logger.info(f"Nmap scanning {ip} with args: {args}")

            msg = await update.message.reply_text(
                f"🔍 Nmap scanning **{ip}** [0%] (ETA: {eta_text}, Types: {', '.join(scan_types)})",
                parse_mode="Markdown"
            )
            ...
            try:
                ...
                await context.bot.edit_message_text(
                    ...
                    text=f"⚠️ Nmap scan timed out for **{ip}**. Try a single scan like -sT.",
                    parse_mode="Markdown"
                )
                return
            ...
            await update_buttons(chat_id, context, ip, 100, 0, "nmap")
            ...
            logger.error(f"Nmap PortScannerError for {ip}: {str(e)}\nStack trace: {traceback.format_exc()}")
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_ids[chat_id],
                text=f"⚠️ Nmap scan failed for **{ip}**: {str(e)}\nTry TCP Connect (-sT) or Version Detection (-sV).",
                parse_mode="Markdown"
            )
        except (MemoryError, OSError) as e:
            logger.error(f"Nmap resource error for {ip}: {str(e)}\nStack trace: {traceback.format_exc()}")
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_ids[chat_id],
                text=f"⚠️ Nmap scan failed for **{ip}**: Resource limit hit. Try a lighter scan like -sT.",
                parse_mode="Markdown"
            )
        except Exception as e:
            ...
            multi_scan_selections.pop(chat_id, None)
            lock_timeouts.pop(chat_id, None)
...
async def scan_single_ip(ip, chat_id, update, context, is_cctv=False):
    ...
    async with scan_semaphore:
        try:
            scan_locks[chat_id] = True
            lock_timeouts[chat_id] = time.time() + 1200  # 20 min timeout
            scan_stop[chat_id] = False
            scan_results[chat_id] = {"open": [], "closed_count": 0, "details": {}, "mac": "N/A", "os": "Unknown"}
            scan_expiry[chat_id] = time.time() + 600
            ...
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_ids[chat_id],
                text=f"⚠️ IP scan failed for **{ip}**: {str(e)}",
                parse_mode="Markdown"
            )
        finally:
            scan_locks.pop(chat_id, None)
    ...
    try:
        if query.data.startswith("ip_scan_"):
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("SYN Scan (-sS) [Privileged, may fail]", callback_data=f"nmap_sS_{chat_id}")],
                [InlineKeyboardButton("UDP Scan (-sU) [Privileged, may fail]", callback_data=f"nmap_sU_{chat_id}")],
                [InlineKeyboardButton("OS Detection (-O) [Privileged, may fail]", callback_data=f"nmap_O_{chat_id}")],
                [InlineKeyboardButton("Aggressive (-A) [Privileged, may fail]", callback_data=f"nmap_A_{chat_id}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.message.reply_text(
                "🔍 Select Nmap scan type (privileged scans may fail on free hosting):",
                reply_markup=reply_markup
            )
        elif query.data.startswith("multi_nmap_"):
            multi_scan_selections[chat_id] = []
            await show_multi_nmap_menu(chat_id, query.message, context)
        elif query.data.startswith("multi_select_"):
            scan_type = query.data.split("_")[2]
            if scan_type in multi_scan_selections[chat_id]:
                multi_scan_selections[chat_id].remove(scan_type)
            elif len(multi_scan_selections[chat_id]) < 2:
                multi_scan_selections[chat_id].append(scan_type)
            else:
                await query.message.reply_text("⚠️ Max 2 scan types allowed.")
                return
            await show_multi_nmap_menu(chat_id, query.message, context)
        elif query.data.startswith("multi_confirm_"):
            if not multi_scan_selections.get(chat_id):
                await query.message.reply_text("⚠️ Please select at least one scan type.")
                return
            awaiting_input[chat_id] = "multi_nmap"
            await query.message.reply_text(
                f"🔧 Enter an IP for Multi-Nmap scan ({', '.join(multi_scan_selections[chat_id])}):",
                parse_mode="Markdown"
            )
...
# Multi-Nmap menu
async def show_multi_nmap_menu(chat_id, message, context):
    scan_options = ["sT", "sV", "sC", "sS", "sU", "O", "A"]
    keyboard = []
    for opt in scan_options:
        label = f"{'✅' if opt in multi_scan_selections.get(chat_id, []) else '⬜'} {opt}"
        if opt in ["sS", "sU", "O", "A"]:
            label += " [Privileged, may fail]"
        keyboard.append([InlineKeyboardButton(label, callback_data=f"multi_select_{opt}_{chat_id}")])
    keyboard.append([InlineKeyboardButton("✔️ Confirm Selection (Max 2)", callback_data=f"multi_confirm_{chat_id}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    try:
        await message.edit_text(
            "🔧 Select up to 2 Nmap scan types (privileged scans may fail on free hosting):",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Error updating multi-nmap menu: {e}")
        await message.reply_text("🔧 Select up to 2 Nmap scan types (privileged scans may fail on free hosting):", reply_markup=reply_markup)
...
async def scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif mode == "multi_nmap":
            if "/" not in target:
                await nmap_scan(target, chat_id, update, context, multi_scan_selections[chat_id])
            else:
                await update.message.reply_text("⚠️ Multi-Nmap scans support single IPs only. Use `192.168.1.1`.")
    except Exception as e:
                ...
                try:
                    if mode == "ip_scan":
                        ...
                        if "/" not in target:
                            ...
                            await app.bot.send_message(
                                chat_id=chat_id,
                                text="⚠️ Multi-Nmap scans support single IPs only. Use `192.168.1.1`."
                            )
                finally:
                    scan_queue.task_done()
