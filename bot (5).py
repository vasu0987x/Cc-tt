...
SERVICE_MAP = {
    ...
    22: "ssh", 23: "telnet", 443: "https", 445: "smb", 3389: "rdp"
}

# Port usage descriptions
PORT_USAGE = {
    80: "Access via browser",
    554: "Stream via VLC",
    8000: "Access via browser",
    8080: "Access via browser",
    8443: "Access via browser (secure)",
    37020: "ONVIF device discovery",
    22: "Remote shell access",
    23: "Telnet access",
    443: "Access via browser (secure)",
    445: "File sharing",
    3389: "Remote desktop"
}
...
async def validate_rtsp(ip: str, port: int, chat_id: int) -> tuple[bool, str]:
    try:
        ...
        logger.error(f"RTSP validation error for {ip}:{port}: {str(e)}")
        return False, str(e)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🎥 **CCTV Hacking**: Scan camera ports (80, 554, 8000, 8080, 8443, 37020) (~1-2 sec per IP)",
        parse_mode="Markdown",
...
async def scan_port(ip, port, chat_id, protocol="tcp"):
    try:
        ...
        sock.close()
        logger.debug(f"Scanned {ip}:{port} ({protocol}): {'open' if result == 0 else 'closed'}")
        return port, result == 0, protocol
    except socket.error as e:
        logger.error(f"Error scanning port {port} ({protocol}) on {ip}: {str(e)}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error scanning port {port} ({protocol}) on {ip}: {str(e)}\n{traceback.format_exc()}")
        return None
...
async def scan_single_ip(ip, chat_id, update, context, is_cctv=False):
    ...
    if cached:
        ports = [
            f"Port {port} ({SERVICE_MAP.get(port, 'unknown')}) - {PORT_USAGE.get(port, 'Unknown usage')}"
            for port, proto in cached["open"]
        ]
        await update.message.reply_text(
    ...
    async with scan_semaphore:
        try:
                ...
                try:
                    ...
                    for result in results:
                        ...
                        if isinstance(result, Exception):
                            logger.error(f"Port scan exception: {str(result)}\n{traceback.format_exc()}")
                            continue
                        if result:
                            ...
                            if is_open:
                                ...
                                if is_cctv and protocol == "tcp" and port == 554:
                                    try:
                                        ...
                                        if is_valid:
                                            details["rtsp"] = f"Stream via VLC (rtsp://{ip}:{port}/live)"
                                    except Exception as e:
                                        logger.error(f"RTSP scan error for {ip}:{port}: {str(e)}")
                                if is_cctv and protocol == "udp" and port == 37020:
                                    details["onvif"] = "ONVIF device discovery"
                                scan_results[chat_id]["details"][(port, protocol)] = details
                            else:
                                scan_results[chat_id]["closed_count"] += 1
                        if completed % update_interval == 0 or is_cctv:
                    ...
                    logger.error(f"Batch scan error for {ip}: {str(e)}\n{traceback.format_exc()}")
                    continue
                finally:
                    for task in tasks:
                        cancel_tasks.discard(task)
            ...
            if scan_results[chat_id]["open"]:
                ports = [
                    f"Port {port} ({SERVICE_MAP.get(port, 'unknown')}) - {PORT_USAGE.get(port, 'Unknown usage')}"
                    for port, proto in sorted(scan_results[chat_id]["open"])
                ]
                result_msg = f"🟢 Open Ports for **{ip}**:\n" + "\n".join(ports) + f"\nMAC: {scan_results[chat_id]['mac']}"
...
async def update_buttons(chat_id, context, ip, progress, eta, scan_type, completed=False):
    ...
    try:
        logger.info(f"Updating progress for {ip}: {progress:.1f}%, open: {open_ports}, closed: {closed_ports}")
        await context.bot.edit_message_text(
            chat_id=chat_id,
            message_id=message_ids[chat_id],
            text=progress_text,
        ...
        if "Message is not modified" not in str(e):
            logger.error(f"Error updating buttons for {ip}: {str(e)}\n{traceback.format_exc()}")
            ...
            if action == "open":
                ...
                if not open_ports:
                    ...
                    for port, protocol in sorted(open_ports):
                        port_info = f"Port {port} ({SERVICE_MAP.get(port, 'unknown')}) - {PORT_USAGE.get(port, 'Unknown usage')}"
                        if (port, protocol) in scan_results[chat_id]["details"]:
                            details = scan_results[chat_id]["details"][(port, protocol)]
                            if "rtsp" in details:
                                port_info += f"\n  - {details['rtsp']}"
                            elif "onvif" in details:
                                port_info += f"\n  - {details['onvif']}"
