...
def detect_service_version(ip, port, protocol):
    try:
        if protocol == "tcp" and port in [80, 8080, 8000, 8443]:
            model = detect_camera_model(ip, port)
            return model if "Unknown" not in model else "http"
        elif protocol == "tcp" and port == 554:
            return "rtsp"
        elif protocol == "udp" and port == 37020:
            return "onvif"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🔍 **Nmap Advanced Scan**: Scan top 1000 ports with options (~1-2 min)\n"
        "🔧 **Multi-Nmap Scan**: Combine up to 2 scan options (~1-2 min)",
        parse_mode="Markdown",
...
# Nmap-style scan using socket logic
async def nmap_scan(ip, chat_id, update, context, scan_types):
    ...
    async with scan_semaphore:
        try:
            ...
            # Define scan ports (top 1000 for speed)
            top_ports = list(range(1, 1001))  # Common ports
            scan_ports = []
            ...
            if "sU" in scan_types:
                scan_ports.extend([(p, "udp") for p in UDP_PORTS])
            if "sT" in scan_types or not scan_types:
                scan_ports.extend([(p, "tcp") for p in top_ports])
            total_ports = len(scan_ports)
            eta_text = "~1-2 min"

            logger.info(f"Nmap-style scanning {ip}: {total_ports} ports (Types: {', '.join(scan_types or ['sT'])})")

            msg = await update.message.reply_text(
                f"🔍 Nmap scanning **{ip}** [0%] (ETA: {eta_text}, Types: {', '.join(scan_types or ['sT'])})",
                parse_mode="Markdown"
            )
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
                ...
                completed = 0
                update_interval = max(total_ports // 10, 100)
                for future in concurrent.futures.as_completed(futures):
                    ...
                    if result:
                        ...
                        if is_open:
                            ...
                            details = {}
                            if "sV" in scan_types:
                                details["service"] = detect_service_version(ip, port, protocol)
                            if "sC" in scan_types and protocol == "tcp":
                                if port in [80, 8080, 8000, 8443]:
                                    details["creds"] = test_default_creds(ip, port)
                                    details["onvif"] = test_onvif(ip, port)
                                if port == 554:
                                    details["rtsp"] = test_rtsp_brute(ip, port)
                            if "sC" in scan_types and protocol == "udp" and port == 37020:
                    ...
                    if completed % update_interval == 0:
                        ...
                        await update_buttons(chat_id, context, ip, progress, 0, "nmap")
            ...
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_ids[chat_id],
                text=f"⚠️ Nmap scan failed for **{ip}**: {str(e)}\nTry a different IP or fewer scan types.",
                parse_mode="Markdown"
            )
        finally:
            scan_locks.pop(chat_id, None)
    ...
    try:
        if query.data.startswith("ip_scan_"):
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("UDP Scan (-sU) [Slower]", callback_data=f"nmap_sU_{chat_id}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.message.reply_text(
...
async def show_multi_nmap_menu(chat_id, message, context):
    ...
    try:
        await message.edit_text(
            "🔧 Select up to 2 Nmap scan types:",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Error updating multi-nmap menu: {e}")
        await message.reply_text("🔧 Select up to 2 Nmap scan types:", reply_markup=reply_markup)
